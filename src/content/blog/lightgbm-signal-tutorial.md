...
先に気になる点を。**LightGBMは完全無料で使えます。** Microsoftが開発したオープンソース(MITライセンス)のライブラリで、個人利用でも商用利用でも料金は一切かかりません。pipでインストールするだけで、そのまま使えます。

「機械学習=高価なツールやクラウド課金が必要」というイメージがあるかもしれませんが、LightGBM自体はゼロ円です。学習も手元のPCで完結するので、クラウドの利用料もかかりません(大規模データでなければVPS上でも十分動きます)。

## なぜLightGBMなのか

無料であること以外にも、ボットの売買シグナルにLightGBM(勾配ブースティング木)がよく使われる理由があります。
